...
| 6–8. Status wording, colours and hash visibility differ by screen | **In progress**: dashboard (4a), the send flow, Status page and History (4b), and the wallet with its drawer (4c) now use `transaction_badge` / `cashout_badge` / `amount()` / `tx_hash()`, and so do the cash-out detail page's badge and hash. The rest of the cash-out flow (4d) and the admin pages (4e) follow. |
