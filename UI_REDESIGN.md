...
### 7.5 Cash-out flow — ✅ 4d

*As built (4d):*
- **Stepper:** Amount → Review → Status (`CASHOUT_STEPS`), for `/cashout` → POST `/cashout/preview` → `/cashout/{id}`.
- **Amount:** a large UCTUSD field (name and id unchanged) capped at 9 digits and 6 decimals, the same way as the send flow: `pattern`/`maxlength` for the no-JS submit, and characters rejected as you type. **Max** copies the server's available figure into the field, with no arithmetic in the browser. The balance card shows the amount held by requests awaiting approval and the amount available.
- **Review:** the shared breakdown (UCTUSD to burn, fee with its % and minimum, rate, and **You receive** as the hero, counting up), plus a "What happens next" timeline. The hidden price-lock inputs are unchanged (the perf regex still matches). Confirming now redirects to the request's Status page instead of My cash-outs, as the send flow does after paying.
- **Status (`/cashout/{id}`):**
  - a hero showing the simulated payout, with the badge and the check;
  - a requested → approved → completed timeline whose current step pulses; "Awaiting ledger confirmation" is the current step's text while the burn is unconfirmed;
  - the amounts card with the explorer-linked burn hash.
  Everything on it updates live through `/status`. A **failed** cash-out shows Requested ✓ → **Cash-out failed** ✗, with the reason in a red alert and the payout struck through. The data can't tell a rejection from a burn that failed after approval (`reject` also stamps `approved_at`), so the page never claims an approval.
- **My cash-outs:** uses the wallet's rows (shared `recipient/_activity.html`), with the fiat payout under each amount, live polling and an empty state.
- **Live updates:** timelines are redrawn only when the status actually changes (`data-status-sig`), so a poll never overwrites a real date with "just now".
